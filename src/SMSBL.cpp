/*
 * SMSBL.cpp
 * Feit SMSBL series serial servo application layer program
 * Date: 2020.6.17
 * Author.
 */
#include "SMSBL.h"
...
int SMSBL::writeSpeed(u8 ID, s16 Speed, u8 ACC)
{
  if (Speed < 0)
  {
    Speed = -Speed;
...
int SMSBL::enableTorque(u8 ID, u8 Enable) { return writeByte(ID, SMSBL_TORQUE_ENABLE, Enable); }

int SMSBL::unlockEprom(u8 ID) { return writeByte(ID, SMSBL_LOCK, 0); }

int SMSBL::lockEprom(u8 ID) { return writeByte(ID, SMSBL_LOCK, 1); }

int SMSBL::calibrationOfs(u8 ID) { return writeByte(ID, SMSBL_TORQUE_ENABLE, 128); }
...
int SMSBL::readTemperature(int ID)
{
  int Temper = -1;
  if (ID == -1)
  {
